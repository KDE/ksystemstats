    ...
    : NetworkDevice(id, name)
{
    ...
    std::vector<KSysGuard::SensorProperty *> statisticSensors{m_downloadSensor,
                                                              m_downloadBitsSensor,
                                                              m_totalDownloadSensor,
                                                              m_uploadSensor,
                                                              m_uploadBitsSensor,
                                                              m_totalUploadSensor};
    ...
    for (auto property : statisticSensors) {
        connect(property, &KSysGuard::SensorProperty::subscribedChanged, this, [this, statisticSensors](bool subscribed) {
            ...
            } else if (std::none_of(statisticSensors.begin(), statisticSensors.end(), [](auto property) {
                           return property->isSubscribed();
                       })) {
...
void SysctlBackend::start()
{
    ...
    for (i = 1; i <= count; i++) {
        ...
        if ((ifmd.ifmd_data.ifi_type == IFT_ETHER) || (ifmd.ifmd_data.ifi_type == IFT_IEEE80211)) {
            ...
            for (j = 0; j < 6; j++) {
                device->m_sysctl_name[j] = sname[j];
            }
